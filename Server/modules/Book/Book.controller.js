...
export const borrowBook = async (req, res, next) => {
    ...
    const Borrower = await UserModel.findByIdAndUpdate(_id, {
    ...
    if (borrowedBook) {
        ...
        if (sendingEmail && sendingUserEmail)
            return res
                .status(200)
...
export const returnBook = async (req, res, next) => {
    const { _id } = req.user;
    const { bookId } = req.params;
    const borrowingRecord = await BookModel.findOne({
        _id: bookId,
        BorrowedBy: _id,
        isBorrowed: true,
    });
    ...
    const daysLate =
        returnDate.diff(moment(borrowingDate), 'days') - borrowingPeriod;
    const fine = daysLate > 0 ? daysLate * borrowingRecord.finePerDay : 0;
    const book = await BookModel.findOneAndUpdate(
        { _id: bookId, BorrowedBy: _id, isBorrowed: true },
        {
            isBorrowed: false,
            $unset: { BorrowedBy: '', BorrowedAt: '', BorrowedPeriod: '' },
        }
    );
    const user = await UserModel.findOneAndUpdate(
        { _id, isLoggedIn: true, confirmed: true },
        { fines: fine, $pull: { borrowedBooks: bookId } }
    );
...
export const searchBorrowedBooks = async (req, res, next) => {
    ...
    const books = await BookModel.find({
        ...
        BorrowedAt: { $gte: moment(borrowingDate) },
    });
...
export const uploadBookPicture = async (req, res, next) => {
    ...
    const updatedBook = await BookModel.findOneAndUpdate(
        { _id: BookId },
        { BookPictures: [...book.BookPictures, ...coverPicsArr] },
        { new: true }
    );
    updatedBook
        ? res.status(200).json({ message: 'Done', UpdatedData: updatedBook })
        : next(new Error('Error Uploading Photos'));
};
