...
import {
    comparingFunction,
    hashingFunction,
} from '../../Utils/HashingFunction.js';
import { tokenDecode, tokenGeneration } from '../../Utils/TokenFunction.js';
import { nanoid } from 'nanoid';

export const signUp = async (req, res, next) => {
    const { name, email, password, phone, age, isAdmin } = req.body;
    ...
    const token = tokenGeneration({
        payload: { id: user._id, name: user.name, isAdmin: user.isAdmin },
    });
    ...
    if (sendingEmail) {
        ...
        return res.status(201).json({
            message: 'Signup Success, Please Confirm Your Email Then Login',
            User: savedUser,
        });
    }
...
export const sendForgetPasswordEmail = async (req, res, next) => {
    ...
    await UserModel.findOneAndUpdate(
        ...
        { resetPasawordCode: resetCode, resetExpiresTimer: resetExpires }
    );
...
export const ResetPassword = async (req, res, next) => {
    ...
    if (
        ...
        Date.now() > user.resetExpiresTimer
    )
        return res
            .status(400)
